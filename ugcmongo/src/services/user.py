...
class UserService:
    ...
    async def add_user_bookmark(self, user_id: str, film_id: str) -> Bookmark:
        ...
        if bookmark:
            return Bookmark(user_id=bookmark['user_id'],
                            movie_id=bookmark['movie_id'])
        await self.collection.insert_one({'user_id': user_id,
                                          'movie_id': film_id})
        return Bookmark(user_id=user_id, movie_id=film_id)

    async def remove_user_bookmark(
            self, user_id: str, film_id: str) -> Bookmark or None:
...
def get_user_service(
        ...
        mongo: AsyncIOMotorClient = Depends(get_mongo)) -> UserService:
