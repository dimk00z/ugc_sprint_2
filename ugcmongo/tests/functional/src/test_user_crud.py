...
from tests.functional.utils.extract import (extract_bookmark,
                                            extract_bookmarks, extract_message)
...
async def test_user_bookmarks_endpoint_crud(
    ...
    response = await make_post_request('user/bookmark',
                                       json={'user_id': 'test-usr-uuid',
                                             'movie_id': 'test-film-uuid0'})
    ...
    response = await make_delete_request('user/bookmark',
                                         json={'user_id': 'test-usr-uuid',
                                               'movie_id': 'test-film-uuid0'})
    bookmark = await extract_bookmark(response)
    assert response.status == HTTPStatus.OK
