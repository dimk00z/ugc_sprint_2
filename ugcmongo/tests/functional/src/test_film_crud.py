...
async def test_film_endpoint_crud(
    ...
    response = await make_post_request(
        "film/vote",
        json={"user_id": "user-uuid-1", "movie_id": "test-film-uuid", "rating": 1},
    )
    ...
    response = await make_post_request(
        "film/vote",
        json={"user_id": "user-uuid-1", "movie_id": "test-film-uuid", "rating": 10},
    )
    vote = await extract_film_vote(response)
    assert vote.user_id == "user-uuid-1"
    assert vote.movie_id == "test-film-uuid"
    ...
    response = await make_post_request(
        "film/vote",
        json={"user_id": "user-uuid-2", "movie_id": "test-film-uuid", "rating": 5},
    )
    vote = await extract_film_vote(response)
    ...
    response = await make_post_request(
        "film/vote",
        json={"user_id": "user-uuid-1", "movie_id": "test-film-uuid2", "rating": 3},
    )
    vote = await extract_film_vote(response)
    assert vote.user_id == "user-uuid-1"
    ...
    response = await make_delete_request(
        "film/vote", json={"user_id": "user-uuid-1", "movie_id": "test-film-uuid2"}
    )
    vote = await extract_film_vote(response)
    assert vote.user_id == "user-uuid-1"
    assert vote.movie_id == "test-film-uuid2"
    ...
    response = await make_delete_request(
        "film/vote", json={"user_id": "user-uuid-1", "movie_id": "test-film-uuid"}
    )
    vote = await extract_film_vote(response)
    assert vote.user_id == "user-uuid-1"
    assert vote.movie_id == "test-film-uuid"
    ...
    # Remove vote
    response = await make_delete_request(
        "film/vote", json={"user_id": "user-uuid-2", "movie_id": "test-film-uuid"}
    )
    vote = await extract_film_vote(response)
    assert vote.user_id == "user-uuid-2"
    assert vote.movie_id == "test-film-uuid"
...
async def test_film_review_endpoint_crud(
    ...
    response = await make_post_request(
        "film/review/info",
        json={"user_id": "test-user-uuid", "movie_id": "test-film-uuid"},
    )
    message = await extract_message(response)
    assert response.status == HTTPStatus.NOT_FOUND
    assert message.detail == "Not Found"
    ...
    response = await make_post_request(
        "film/vote",
        json={"user_id": "test-user-uuid", "movie_id": "test-film-uuid", "rating": 7},
    )
    vote = await extract_film_vote(response)
    ...
    # Delete review
    response = await make_delete_request(
        "film/review", json={"user_id": "test-user-uuid", "movie_id": "test-film-uuid"}
    )
    review = await extract_film_review(response)
    assert review.user_id == "test-user-uuid"
    assert review.movie_id == "test-film-uuid"
    ...
    # Remove vote
    response = await make_delete_request(
        "film/vote", json={"user_id": "test-user-uuid", "movie_id": "test-film-uuid"}
    )
    vote = await extract_film_vote(response)
    assert vote.user_id == "test-user-uuid"
    assert vote.movie_id == "test-film-uuid"
